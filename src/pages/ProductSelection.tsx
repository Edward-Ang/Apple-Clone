...
const ProductSelection: React.FC = () => {
    ...
    return (
        <>
            <Banner content='Buy MacBook Pro with education savings. Plus get AirPods.◊◊◊' />
            <div className="container mx-auto p-12 flex justify-center items-center">
                <div>
                    <h1 className="text-3xl sm:text-5xl font-semibold mb-6 text-center font-stack">Choose you new MacBook Pro.</h1>
                    ...
                    <div className='w-full flex justify-center items-center py-5' style={{ position: 'sticky', top: 0, zIndex: 1, backgroundColor: 'white' }}>
                        <button
                            className={`w-full sm:w-auto font-semibold py-5 px-4 rounded-tl-lg rounded-bl-lg border ${selectedInch === 14 ? 'border-blue-500 border-2' : 'border-black'}`}
                        ...
                        <button
                            className={`w-full sm:w-auto font-semibold py-5 px-4 rounded-tr-lg rounded-br-lg border ${selectedInch === 16 ? 'border-blue-500 border-2' : 'border-black'}`}
                            style={{ maxWidth: '274px', minWidth: '141px', minHeight: '83px', fontSize: '17px' }}
                    ...
                    <div className='flex flex-col justify-center items-center'>
                        ...
                        <div className='flex justify-between items-center pt-8 pb-16'>
                            <button
                                className={`px-3 sm:px-5 py-1 mx-3 rounded-3xl ${activeFilter === 'All' ? 'bg-black text-white' : 'bg-gray-200'}`}
                                onClick={() => handleFilterClick("All")}>All
                            </button>
                           {selectedInch !== 16 ? (
                            <button
                                className={`px-3 sm:px-5 py-1 mx-3 rounded-3xl ${activeFilter === 'M3' ? 'bg-black text-white' : 'bg-gray-200'}`}
                            ...
                            <button
                                className={`px-3 sm:px-5 py-1 mx-3 rounded-3xl ${activeFilter === 'M3 Pro' ? 'bg-black text-white' : 'bg-gray-200'}`}
                                onClick={() => handleFilterClick("M3 Pro")}>M3 Pro
                            </button>
                            <button
                                className={`px-3 sm:px-5 py-1 mx-3 rounded-3xl ${activeFilter === 'M3 Max' ? 'bg-black text-white' : 'bg-gray-200'}`}
                    ...
                    <div className='block sm:flex w-full pb-11'>
                        <div className='flex flex-col justify-center items-center'>
                            <div>
